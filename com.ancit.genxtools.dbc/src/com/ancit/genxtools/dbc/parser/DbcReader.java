...
import com.ancit.genxtools.dbc.model.*;
import com.ancit.genxtools.dbc.parser.AttributeDefinition.AttrType;
...
public class DbcReader {
	...
	public NetworkDefinition getNetwork() {
		if (network == null) {
			...
			System.out.println("no network read");
		}
		return network;
	}
	 ...
	 * @param id
	 *            CAN identifier of the message to find
	 * @param e
	 *            True, if the message to find is of extended frame format
	 ...
	 * @param id
	 *            Identifier of CAN message to find
	 * @param e
	 *            True, if CAN message to find is extended frame format
	 * @param name
	 ...
	 *
	 * @param file
	 *            CAN database filehandle to read.
	 * @param logStream
	 *            OutputStream to write out stack traces
	...
	public boolean parseFile(File file, OutputStream logStream) {
		...
		environmentVariables = (EnvironmentVariables) (factory.createEnvironmentVariables());
		environmentVariables.setName("Enviroment Variable");

		nodesObj = factory.createNodes();
		nodesObj.setName("Nodes");
		...
		try {
			...
			network.getEnvVariables().add(environmentVariables);
			network.getNode().add(nodesObj);
		...
		for (Attribute attr : attributes) {
			...
			case MESSAGE:
				...
				if (m != null) {
					...
					logWriter.println("Attribute " + attr.getName()
							+ " found for not-found message. This looks like a bug in the DBC.");
				}
				break;
			case SIGNAL:
		...
		for (Message m : messages) {
			if(m.getInterval() == 0) {
				AttributeDefinition attributeDefinition = attribute_definitions.get("GenMsgCycleTime");
				if (attributeDefinition != null) {
					m.setInterval((int) ((AttributeDefinitionInt) attributeDefinition).getDefault());
				}
			}
		}
	 ...
	 *            File to save.
	 * @param prettyPrint
	 *            True, to format for human reading.
	 * @param gzip
	 *            True, to compress output file.
	...
	public boolean writeKcdFile(File file, boolean prettyPrint, boolean gzip) {
		...
		try {
			JAXBContext context = JAXBContext
					.newInstance(new Class[] { com.ancit.genxtools.dbc.model.NetworkDefinition.class });
	 ...
	 *
	 * @param line
	 *            Related parts of a dbc-file will be passed over to the suitable
	 *            handling method.
	 */
	...
	private void parseAttributeDefinition(StringBuilder line) {
		...
		if (splitted[0].equals("BA_DEF_")) {
			...
			case INT:
				...
				attribute_definitions.put(name, new AttributeDefinitionInt(name, trgt,
						Long.parseLong(splitted[attr_def_start + 2]), Long.parseLong(splitted[attr_def_start + 3])));
				break;
			case FLOAT:
				if (splitted.length < 5) {
					return;
			...
			case HEX:
				...
				attribute_definitions.put(name, new AttributeDefinitionHex(name, trgt,
						Long.parseLong(splitted[attr_def_start + 2]), Long.parseLong(splitted[attr_def_start + 3])));
				break;
			}
			...
			if (def != null) {
				...
				// DBCs like to set empty default values as empty strings, even if the attribute
	 ...
	 *
	 * @param line
	 *            passed over buffer of the line (starting with BO_ including all
	...
	private void parseEnvironmentVariable(StringBuilder line) {
		String[] envItems = line.toString().split(" ");
		EnvironmentVariable ev = (EnvironmentVariable) factory.createEnvironmentVariable();
		String[] min_max = (envItems[3].replace("[", "").replace("]", "")).split("|");
		ev.setName(envItems[1].replace(":", ""));
		if (envItems[2].equalsIgnoreCase("0")
				&& envItems[7].substring(envItems[7].length() - 4, envItems[7].length() - 1).equalsIgnoreCase("800")) {
			ev.setValuetype("String");
		} else if (envItems[2].equalsIgnoreCase("0")) {
			ev.setValuetype("Integer");
			ev.setInitialValue(Integer.parseInt(envItems[5]));
			ev.setMinimumValue(Integer.parseInt(min_max[0]));
			ev.setMaximumValue(Integer.parseInt(min_max[2]));
		} else if (envItems[2].equalsIgnoreCase("1")) {
			ev.setValuetype("Float");
			ev.setInitialValue(Integer.parseInt(envItems[5]));
			ev.setMinimumValue(Integer.parseInt(min_max[0]));
			ev.setMaximumValue(Integer.parseInt(min_max[2]));
		} else {

			ev.setValuetype("Data");

		}

		if (envItems[7].substring(envItems[7].length() - 1, envItems[7].length()).equalsIgnoreCase("0")) {
			ev.setAccess("unrestricted");
		} else if (envItems[7].substring(envItems[7].length() - 1, envItems[7].length()).equalsIgnoreCase("1")) {
			ev.setAccess("Read");
		} else if (envItems[7].substring(envItems[7].length() - 1, envItems[7].length()).equalsIgnoreCase("2")) {
			ev.setAccess("Write");
		} else if (envItems[7].substring(envItems[7].length() - 1, envItems[7].length()).equalsIgnoreCase("3")) {

			ev.setAccess("ReadWrite");

		}
		ev.setSno(Integer.parseInt(envItems[6]));
		ev.setUnit(envItems[4].replace("\"\"", ""));

		environmentVariables.getEnvirementVariable().add(ev);
	 ...
	 *
	 * @param message
	 *            message object where the signal line belongs to and shall append
	 *            to.
	 * @param signalName
	 *            name of the signal as parsed before the line string begins
	 * @param type
	 *            signal type that is one of multiplexor, multiplex or plain signal.
	 * @param line
	 *            signal line String to parse e.g. "39|16@0+ (0.01,0) [0|655.35]
	 *            "Km/h" ECU3"
	 */
	 ...
	 */
	// protected void parseSignal(Message message, String line) previously
	public void parseSignal(Message message, String line) {
	 ...
	 *
	 * @param line
	 *            line from dbc-file to handle e.g. "VAL_ 1234 signalname 1 "on" 2
	 *            "off" ;".
	 *
	 */
	 ...
	 *
	 * @param line
	 *            line from dbc-file to handle e.g. "CM_ SG_ 1234 signalname 1
	 *            "comment";".
	 */
